...
chunk_size <- 100000  # Size of each chunk for prediction # DOTO ajuster en fonction de la RAM disponible

# Path
date <- "20240905"
folder <- paste0("/project/def-nbl/come/PLANET_8B_", date)
raster_path <- "/scratch/come/composite.tif"
model_path <- "/project/def-nbl/come/models/rf_model_manual.rds"
...
classified_files <- foreach(i = seq_len(length(blocks)), 
  ...
  for (j in seq_len(n_chunks)) {
    ...
    p <- predict(modele_rf, df_chunk)$predictions
  ...
  cat(sprintf("[%s] Bloc %d classifié et sauvé\n", Sys.time(), i), file = log_file, append = TRUE)
  # Memory cleanup
...
writeRaster(final_raster, file.path(folder, paste0("classification_result_", date, ".tif")), overwrite = TRUE)
